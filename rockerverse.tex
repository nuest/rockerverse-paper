...
command line output. \CRANpkg{stevedore}'s own interface is
...
similar to R6 objects, in that an object of class
\code{"stevedore\_object"} has a number of functions attached to it that
...
\CRANpkg{renv} helps users to manage the state of the R library in a
...
\citep{pebesma_simple_2018,cran_rgdal}. \CRANpkg{sf} and \CRANpkg{rgdal}
...
\CRANpkg{googleComputeEngineR} integrates with the R parallelisation
package \CRANpkg{future} \citep{cran_future} to enable this with only a
few lines or R
...
actually needs to be re-executed. \CRANpkg{drake} has been demonstrated
...
Furthermore, \CRANpkg{drake} workflows have been shown to use
\CRANpkg{future} package's function \code{makeClusterPSOCK()} for
...
the \CRANpkg{future} package ensures that the host and worker can
...
invokes the actual application. The R session uses \CRANpkg{plumber} or
...
and more recently \CRANpkg{plumber} (\url{https://www.rplumber.io/}) and
...
customer
\citep[see \CRANpkg{Shiny}-based \href{https://secure.message.t-mobile.com/v1/shiny/is-customer/app/}{demo} further described by ][]{nolis_small_2019},
...
a \CRANpkg{plumber} API, then someone else on the marketing team could
...
framework, it was found that the \CRANpkg{testthat} framework did not
...
of \CRANpkg{RSelenium} or \CRANpkg{drake} (see
Sections~\nameref{rselenium} and \nameref{drake} respectively). In some
cases, e.g., for \CRANpkg{analogsea}, the interaction with the Docker
