...
\author{by Daniel Nüst, Dirk Eddelbuettel, Dom Bennett, Robrecht Cannoodt, Dav Clark, Gergely Daroczi, Mark Edmondson, Colin Fay, Ellis Hughes, Lars Kjeldgaard, Sean Lopp, Ben Marwick, Heather Nolis, Jacqueline Nolis, Hong Ooi, Karthik Ram, Noam Ross, Lori Shepherd, Péter Sólymos, Tyson Lee Swetnam, Nitesh Turaga, Charlotte Van Petegem, Jason Williams, Craig Willis, Nan Xiao}
...
\code{harbor::docker\_run(image = "hello-world")} still works depsite
...
The \emph{Bioconductor} \texttt{Dockerfile} selects the desired version
of R from Rocker, adds required system dependencies, and uses the
\CRANpkg{BiocManager} package for installing appropriate versions of
...
documents. \pkg{liftr} makes the document the main and sole workflow
...
container, i.e., \code{render\_docker("foo.Rmd")}. An RStudio addin even
allows compilation of documents with the single push of a button.
...
run any container, the package \pkg{googleCloudRunner} demonstrates how
R can be used as the control language for one-time or batch processing
...
and from containers. Some packages, e.g., \pkg{containerit}, provide
...
Another example is the package \CRANpkg{golem}, which makes an heavy use
of \pkg{dockerfiler} when it comes to creating the \texttt{Dockerfile}
...
remove machine using SSH. \pkg{AzureContainers} is an umbrella package
...
The prevalence of Docker in industry naturally leads to the use of
...
a \pkg{plumber} API, then someone else on the marketing team could write
...
minute. \pkg{holepunch} also creates a \texttt{DESCRIPTION} file for
essential metadata and dependency specification, and thereby turns turns
any project into a research compendium (see~\nameref{compendia}). The
...
\textbf{Teaching} is a further example of where shared computing
...
Second, the idea of a common environment and partitioning allows for
...
Another example of a Docker-based teaching and testing platform is
\href{https://github.com/PrairieLearn}{PrairieLearn} which is being
developed at the University of Illinois at Urbana-Champaign
\citep{prairielearn:2018} where it has already been in extensive use
across several faculties (along with initial use on some other
campuses). It uses Docker containers a key components, both internally
for its operations (programmed mainly in Python as well as in
Javascript), as well as for two reference containers providing,
respectively, Python and R autograders. A key design decision made by
PrairieLearn permits \emph{external} grading container to be supplied
and accessed via a well-defined interface of invoking, essentially, a
single script \texttt{run.sh} expecting JSON based configuration,
relying on a well-defined file layout containing support files, exam
questions, supplementary data as well as student submissions, and
returning per-question evaluations as JSON result files which are
evaluated, aggregated and recorded in SQL. The
...
container
\citep{rocker-pl:2019}.\footnote{The reference R container was unavailable at the time, and also relies on a heavier CentOS-based build so that a lighter and alternative was established.}
The \texttt{rocker-pl} container extends \texttt{rocker/r-base} with
both the \href{https://github.com/stat430dspm/plr}{\texttt{plr}}
\citep{pkg:plr:2019} custom R package for integration into PrarieLearn
testing and question evaluation, along with the actual R packages used
in instruction and testing for the course in question. As automated
grading of submitted student answers is close to the well-understood
problem of unit testing, the \CRANpkg{tinytest} package
\citep{CRAN:tinytest} is used as it offers both its core features for
testing as well as clean extensibility which package \CRANpkg{ttdo}
\citep{CRAN:ttdo} utilizes to render differences between incorrect
answers and reference answers by invoking the \CRANpkg{diffobj} package
\citep{CRAN:diffobj} for context-sensitive colorized displays of these
differences. The setup, described in more detail by
...
\pkg{rrtools} (\url{https://github.com/benmarwick/rrtools}) provides a
...
audit use case. NX contributed content on \pkg{liftr}. All authors
