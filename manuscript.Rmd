...
Combining both these topics, the Rocker Project ([https://www.rocker-project.org/](https://www.rocker-project.org/)) provides images with R (see details next Section).
The considerable uptake and continued evolution of Rocker images has lead to numerous projects extending or building upon Rocker images, ranging from reproducible research to production deployments.
This article presents this _Rockerverse_ of projects across all development stages: demonstrators, prototypes, and mature products.
...
By allowing multiple concurrent applications or services to run concurrently on one host machine without any fear of interference between them, an important scalability feature had been realized.
...
The key accomplishment of Docker is to make a "bundled" aggregation of software available to any system running Docker, without requiring much else from the host.
This is a rather attractive, and novel, proposition which has lead to widespread adoption and use of Docker in a variety of domains, e.g., cloud computing infrastructure \citep{Bernstein2014}, data science \citep{boettiger_introduction_2015}, and edge computing \citep{alam_orchestration_2018} [more examples!].
