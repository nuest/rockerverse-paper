...
Gigantum builds on the locally running _Gigantum CLI_ and _Gigantum Desktop_ applications for development and execution of analyses, which can be stored and shared via _Gigantum Cloud_.
The Gigantum apps are user friendly interfaces to a backend using Docker containers to package a Gigantum project ([https://github.com/gigantum/base-images](https://github.com/gigantum/base-images)).
...
Furthermore, Gigantum's focus lies on environment management via GUI and ensuring a smooth user interaction, e.g., with reliable and easy conflict detection and resulution.
<!-- @DC: when a user interactively installs packages in Python, do you write that to the Dockerfile? UI-based environment management does not work for R packages, right? \href{https://docs.gigantum.com/docs/environment-management}{https://docs.gigantum.com/docs/environment-management} -->
...
The image build and publication process is scripted with Python and JSON template configuration files, unlike Rocker which relies on more plain `Dockerfile`s.
...
_The ordering of authors following DN as first author is alphabetical._ DN conceived the article idea, \href{https://github.com/nuest/rockerverse-paper/issues/3}{initialised the formation of the writing team}, wrote sections not mentioned below, and revised all sections.
...
CF wrote paragraphs about `r-online`, `dockerfiler`, `r-ci` and `r-db`.
