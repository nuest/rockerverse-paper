...
The R installation is installed using the Conda package manager, which can manage environments for various programming languages, pinning both the R version and the versions of R packages\footnote{See `jupyter/datascience-notebook` `Dockerfile` at \href{https://github.com/jupyter/docker-stacks/blob/master/datascience-notebook/Dockerfile#L47}{https://github.com/jupyter/docker-stacks/blob/master/datascience-notebook/Dockerfile#L47}.}.
...
The `Dockerfile`s also use build args to specify bases, for example for different version of NVIDIA CUDA for GPU processing\footnote{See \href{https://github.com/gigantum/base-images/blob/master/_templates/python3-minimal-template/Dockerfile}{https://github.com/gigantum/base-images/blob/master/\_templates/python3-minimal-template/Dockerfile} for the `Dockerfile` of `python3-minimal`.} so that appropriate GPU drivers can be enabled automatically when supported.
...
One of the first implementations of having R respond to client reqests were TCP/IP and local socket servers, such as \pkg{Rserve} ([https://cran.r-project.org/package=Rserve](https://cran.r-project.org/package=Rserve)) and \pkg{svSocket} ([https://cran.r-project.org/package=svSocket](https://cran.r-project.org/package=svSocket)), which allowed other local or remote processes, even written in other programming languages, such as C/C++ or Java, to make binary requests to interact with R without the need to link to R at compile time.

R could also empower webserver since at least 2007, when [RApache](http://www.rapache.net) was presented at the useR! conference.
...
A similar design was implemented at CARD.com, where the optimization of the acquisition flow and the real-time analysis of debit card transactions both relied on an efficient combination of Rserve and RApache running in Docker containers:
the R session behind Rserve acted as a read-only, exteremely fast and scalabale, in-memory database for the many concurrent RApache processes responding to the live-scoring requests of other components of the company.
Similar dockerized R scripts were responsible for the ETL processes and even the client-facing email, text message and push notification alerts sent in real-time on the card transaction.
The related Docker images were made available at [https://github.com/cardcorp/card-rocker](https://github.com/cardcorp/card-rocker), which are extending `rocker/r-base`, e.g., with an SSH client and workaround for being able to mount SSH keys from the host, Pandoc, the AWS SDK and Java -- which latter was required at that time to be able to easily use the Amazon Web Services via their Java SDK, eg to run R consumers reading from a Kinesis stream, deployed as a dockerized ECS service.
The main learnings from using R in Docker was the imporance of not only pinning the R package versions via MRAN, but also moving away from Debian testing to an LTS distribution to minimize the risk of breaking the automated builds of the Docker images and/or production jobs.
...
The package simplifies complex infrastructure management to a number of R funcction calls, given an Azure account with sufficient credit\footnots{See _"Deploying a prediction service with Plumber"_ vignette at \href{https://cran.r-project.org/web/packages/AzureContainers/vignettes/vig01_plumber_deploy.html}{https://cran.r-project.org/web/packages/AzureContainers/vignettes/vig01_plumber_deploy.html} for details.}.
