...
## Clients [Ellis Hughes]

Interfacing with the Docker daemon is done through the Docker Command Line Interface (CLI).
Moving back and forth between an R session and the command line can create friction in workflows and reduce reproducibility.
To solve this problem there are a number first order R packages exist that provide an interface to the Docker CLI.
This then means that R users are able to take advantage of the benefits of Docker without explicitly working with the CLI.
...
At the time of writing this paper, these packages can work in concert to provide interfaces for installing Docker, building images, and running/controlling the life cycle of docker containers. 

Each of these packages provide functional wrappers to the Docker CLI at different stages of the docker lifecycle. 

_dockyard_ has the goal of lowering barrier to creating dockerfiles and docker images, and deploying docker containers. 
...
_dockyard_ has a 'dockerfile' object that either starts out as an empty dockerfile, or can use existing dockerfile to use as a template.
...
Once the dockerfile is created, _dockyard_ has the functionality to build the image and deploy it.

These clients are important for bringing accessibility to the Docker ecosystem for the wide variety of R programmers.
