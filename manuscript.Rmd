...
**\CRANpkg{AzureContainers}** is an interface to a number of container-related services in Microsoft's [Azure Cloud](https://azure.microsoft.com/), see Section&nbsp;\nameref{azurecontainers}.
...
**\CRANpkg{renv}** ([https://rstudio.github.io/renv/](https://rstudio.github.io/renv/)) with Docker: https://rstudio.github.io/renv/articles/docker.html

_version pinning_ with packages \pkg{remotes} and \pkg{versions} for R, and with system package managers for distros

**\CRANpkg{liftr}** ([https://nanx.me/liftr/](https://nanx.me/liftr/)) \citep{liftr2019} aims to solve the problem of persistent reproducible reporting in statistical computing.
...
the R session behind Rserve acted as a read-only, extremely fast and scalabale, in-memory database for the many concurrent rApache processes responding to the live-scoring requests of various divisions of the company.
...
The main learnings from using R in Docker was the imporance of not only pinning the R package versions via MRAN, but also moving away from Debian testing to an LTS distribution.
For the use case at hand, the priority was to have more control over upstream updates, and to minimize the risk of breaking the automated builds of the Docker images and production jobs.

A more recent related work is the \CRANpkg{plumber} ([https://www.rplumber.io/](https://www.rplumber.io/)) package, which makes it even easier for R to be used to host web services through RESTful APIs.
...
**\CRANpkg{AzureContainers}** is an umbrealla package providing interfaces the three services of Microsoft's Azure Cloud, namely [Container Instances](https://azure.microsoft.com/en-us/services/container-instances/) for running individual containers, [Container Registry](https://azure.microsoft.com/en-us/services/container-registry/) for private image distribution, and [Kubernetes Service](https://azure.microsoft.com/en-us/services/kubernetes-service/) for orchestrated deployments.
...
Another way this idea of a common environment and the partitioning that Docker provides is used in teaching is in the **[Dodona](https://dodona.ugent.be)** project.
Dodona is a web platform developed at Ghent University and is used to teach students basic programming skills.
Dodona uses Docker containers to test submissions by students.
...
New releases of even just a single piece of software in this pipeline can break the entire workflow, making it difficult to find the error and for others to reuse existing pipelines, and making the original the results irreproducible and not extandable.
...
\citet{gentleman_statistical_2007} coined the term _Research Compendium_ for a dynamic document together with supporting data and code.
...
A future researchers can download then compendium from the repository and run the included `Dockerfile` build a new image that recreates the computational environment used to produce the original research results.
If building the image fails, the human-readable instructions in a `Dockerfile` are a the starting point for rebuilding the environment.
...
Reproducible environments, scalability and efficiency, and portability across clouds are the common themes behind R packages, use cases, and applications in this work.
...
The ordering of authors following DN as first author is alphabetical.
