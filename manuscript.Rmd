...
The article continues with a brief introduction of containerisation basics and the Rocker&nbsp;Project, followed by use cases and applications, starting with the R packages specificly for interacting with Docker, next the second-level packages that use containers indirectly or only for specific features, and finally some complex use cases that leverage containers.
...
This complexity and an inherent concern for openness and reproducibility in the data science community led to Docker being used widely.
...
These images are accompanied with a special launch script so the tested source code is not stored in the image; instead, the currently checked in version from a local Git repository is cloned into the container at runtime.
...
[CyVerse](https://cyverse.org) is an open source, NSF-funded cyberinfrastructure platform for the life sciences \citep{merchant_iplant_2016}.
CyVerse supports a browser-based 'data science workbench' called the [Discovery Environment (DE)](https://cyverse-de.github.io/), which uses a combination of [HTCondor](https://research.cs.wisc.edu/htcondor/) and Kubernetes for orchestrating container based analysis, it integrates with external HPC (High Performance Computing), NSF-XSEDE through [TAPIS](https://www.tacc.utexas.edu/tapis) API's.
In addition, CyVerse provides a multi-petabyte Data Store based on [iRODS](https://irods.org/), this allows users to share and manage hundreds of GB's of data.
The DE uses an [OAuth](https://oauth.net/2/) system for single sign on (SSO) for its 70+ thousand registered users.
...
The DE runs Docker containers on demand, with users able to integrate containers from DockerHub or other registries \citep{devisetty_bringing_2016}.
...
The Rocker Project's RStudio and Shiny containers are integrated into the DE by deriving new images from Rocker images.
These images include a reverse proxy using `nginx` to handle communication with CyVerse authentication systems.
All running instances are protected through this authentication system and users can invite other registered users to securely access the same instance \citep{rstudio_proxy_2020}.
Users can reference the same base image and add packages to further customize their R environment.
The Rocker base image is cached on all nodes in the Kubernetes environment to speed up launching.
CyVerse maintains its own [GitHub Organization](https://github.com/cyverse-vice/) with Dockerfiles and configuration scripts auto-built on DockerHub.
For additional reproducibility, CyVerse operates a private Docker image registry where a user's published, i.e., publicly available, containers are held.
CyVerse user accounts are free (with some limitations for non-US users) and the documentation repository [CyVerse Learning Center](https://learning.cyverse.org/) allows community members to document their containers to support training and education.
...
They used the R package system \cite{core_writing_1999} for the functional prototype all the way to structuring, validation, and distribution of research compendia.
...
Furthermore, a maturing of the Rockerverse packages for managing containers may lead to them being adopted in situations where manual coding is currently required, e.g. in the case of \CRANpkg{RSelentium} or \CRANpkg{drake} (see Sections&nbsp;\nameref{rselenium} and \nameref{drake} respectively).
In specific cases, e.g., for \CRANpkg{analogsea}, the interaction with the Docker daemon may remain too specific to re-use first-order packages to control Docker.
