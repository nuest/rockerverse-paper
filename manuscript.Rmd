...
Parallel efforts to enable R for different cloud providers leads to some level of code fragmentation. This fact and to avoid lock-in motivated the [OpenFaaS project](https://www.openfaas.com/), that facilitates the deployment of functions and microservices to Kubernetes or Docker Swarm. OpenFaaS is language agnostic, it provides auto-scaling, metrics, and an API gateway.
...
The templates use [multi-stage Docker builds](https://docs.docker.com/develop/develop-images/multistage-build/) to combine R base images with the OpenFaaS 'watchdog', a tiny Golang webserver that marshals an HTTP request and invokes the application. 
The R session uses \CRANpkg{plumber} or similar packages for the API endpoint with packages and data preloaded, thus minimizing response times.
