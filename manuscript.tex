...
with R}
\author{by Daniel Nüst, Dom Bennett, Robrecht Cannoodt, Dav Clark, Gergely Daroczi, Dirk Eddelbuettel, Mark Edmondson, Colin Fay, Ellis Hughes, Sean Lopp, Ben Marwick, Heather Nolis, Jacqueline Nolis, Hong Ooi, Karthik Ram, Noam Ross, Lori Shepherd, Nitesh Turaga, Craig Willis, Nan Xiao}
...
\begin{figure}
\centering
\includegraphics{/rockerverse/rockerverse.png}
...
dependencies such as \pkg{sf}. The latter is quite comfortable and fast
...
Section~\nameref{conclusion}). Both approaches enable \texttt{apt} as an
...
\pkg{stevedore}'s own interface is automatically generated based on the
...
\textbf{\pkg{renv}} (\url{https://rstudio.github.io/renv/}) with Docker:
\url{https://rstudio.github.io/renv/articles/docker.html}

\emph{version pinning} with packages \pkg{remotes} and \pkg{versions}
...
local socket servers, such as \CRANpkg{Rserve} and \CRANpkg{svSocket},
...
A more recent related work is the \pkg{plumber}
...
\label{azurecontainers} \textbf{\pkg{AzureContainers}} is an umbrealla
...
\pkg{plumber} provides the infrastructure for turning an R workflow a
...
\pkg{AzureContainers} provides a lightweight yet powerful interface for
...
original use case (see~\nameref{introduction}), Docker has no standard
...
(results) to and from containers. Some packages, e.g. \pkg{containerit},
...
\pkg{r-online}, \pkg{dockerfiler}, \pkg{r-ci} and \pkg{r-db}. BM wrote
...
section about \pkg{wholepunch}. NR wrote paragraphs about shared work
...
reproducibility audit use case. NX wrote the section on \pkg{liftr}. All
...
Department of Biological and Environmental Sciences\\ Carl Skottsbergs gata 22B\\ 413 19 Gothenburg, Sweden\\ \orcid{0000-0003-2722-1359}\\
}
