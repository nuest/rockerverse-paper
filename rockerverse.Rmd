...
Two use cases demonstrate the practical usefulness and advantages of containerisation in the context of teaching. On the one hand a special case of shared computing environments (see Section&nbsp;\ref{workenvs}), and on the other hand leveraging sandboxing and controlled environments for autograding.
...
This script relies on a well-defined file layout containing JSON-based configuration, support files, exam questions, supplementary data, and student submissions.
...
The package \CRANpkg{ttdo} \citep{CRAN:ttdo} utilizes the extensibility of \pkg{tinytest} to display context-sensitive colorized differences between incorrect answers and reference answers using the \CRANpkg{diffobj} package \citep{CRAN:diffobj}.
